...
|---|---|---|---|
| SW-FR-001 | VERIFIED_HOST | `Measurement` 包含 UTC 时间、通道、值、单位、状态、来源、质量和 schema；DC/迟滞 runners 保留全部完整/部分记录，校准应用创建可追溯派生 Measurement | Step 7 导出完整记录链 |
| SW-FR-002 | VERIFIED_HOST | `TestRunMetadata` 保存测试、配置、UTC、软件、设备/profile、来源和 raw IDs；DC/迟滞 runners 将完整、部分、unsupported 和 error 生命周期映射为证据一致的 `TestRunResult` | Step 7 写入版本化导出 |
...
| SW-FR-004 | VERIFIED_HOST | Measurement 强制 `record_id`/`raw_record_id`；DC/迟滞分析保留逐点引用，Step 6 校准测试确认拟合不改输入且应用系数只创建保留 raw ID、来源、时间、状态和质量的新记录 | Step 7 把原始/派生引用写入导出 |
...
| SW-FR-034 | VERIFIED_HOST | `calibration-analysis.v1` 产生不可变版本化线性系数、双来源与参与拟合的 record/raw ID、校准前后误差；应用系数生成新 Measurement 而不修改原始批次 | 标准器/实物精度仍需未来 BENCH；Step 7 导出系数和链路 |
...
| SW-FR-036 | VERIFIED_HOST | 公共质量层逐项映射缺失、非有限、饱和、超范围、时间、通信和设备故障；DC、迟滞、校准和频响均保存组件排除决定且禁止由缺失数据形成完整结果 | Step 7 保留所有逐点原因 |
| SW-FR-037 | VERIFIED_HOST | DC 与迟滞 criteria/evaluator 只对完整证据给 PASS/FAIL；校准/频响不产生 criteria 结论，只返回完整或明确 missing/歧义；runners 仍要求 cleanup 成功 | Step 7 导出 conclusion 与 missing requirements |
...
| SW-NFR-004 | VERIFIED_HOST | 单元、黄金、架构、adapter/workflow、runners、校准和频响全部无需硬件；Step 6 两个模块无 adapter/I/O/仪器依赖，Simulator/CSV 仍只读 | Step 7 继续保持离线可测 |
...
| SW-NFR-012 | VERIFIED_HOST | Measurement、capability、TestRun、AFE、配置、公共/DC/迟滞/校准/频响 analysis、criteria、evaluation 和 runners 均显式版本化；Phase 2 意义由黄金文件冻结 | Phase 3 Step 8 冻结新增分析 API/结果 |
...
Software Phase 1 已完成版本化核心，Software Phase 2 已完成 8/8。Software Phase 3 Step 6 已通过 48 项专门测试、992 项完整回归和 100% 正式 package 覆盖；不可变校准系数、派生记录链、前后误差和显式点频响/截止规则已进入正式 package。下一步是 Step 7 版本化 CSV/JSON 结果导出。硬件仍为 DEFERRED，VERIFIED_BENCH 仍为 0。
