...
|---|---|---|---|
| SW-FR-001 | VERIFIED_HOST | `Measurement` 包含 UTC 时间、通道、值、单位、状态、来源、质量和 schema；DC/迟滞 runners 保留全部完整/部分记录并核对通道、单位和唯一 ID | Step 6 校准继续使用派生记录链 |
...
| SW-FR-036 | VERIFIED_HOST | 公共质量层逐项映射缺失、非有限、饱和、超范围、时间、通信和设备故障；DC 保存组件/饱和原因，迟滞缺失点保留逐点排除并禁止发布部分阈值 | Step 6 校准/频响继续复用 |
| SW-FR-037 | VERIFIED_HOST | DC 与迟滞 criteria/evaluator 只对完整证据给 PASS/FAIL；两类 runners 进一步要求 cleanup 成功，任何 unsupported、中止、EOF、执行或清理故障均不能进入 PASS | Step 6 新分析保持相同结论语义 |
...
| SW-NFR-004 | VERIFIED_HOST | 单元、黄金、架构、adapter/workflow 及 DC/迟滞 runners 全部无需硬件；settle/abort 和输出 references 均依赖注入，Simulator/CSV 仍只读 | Step 6 继续维持纯离线分析 |
...
| SW-NFR-012 | VERIFIED_HOST | Measurement、capability、TestRun、AFE、配置、公共/DC/迟滞 analysis、criteria、evaluation 和 runners 均显式版本化；Phase 2 意义由黄金文件冻结 | Phase 3 Step 8 冻结新增分析 API/结果 |
...
Software Phase 1 已完成版本化核心，Software Phase 2 已完成 8/8。Software Phase 3 Step 5 已通过 38 项专门测试、944 项完整回归和 100% 正式 package 覆盖；方向性迟滞证据、逐 cycle 统计、criteria、安全门控 runner 及只读 adapter 的零 I/O 降级已进入正式 package。下一步是 Step 6 校准与离线频率响应。硬件仍为 DEFERRED，VERIFIED_BENCH 仍为 0。
