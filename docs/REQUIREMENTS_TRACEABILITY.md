...
| SW-NFR-004 | VERIFIED_HOST | 单元、黄金、架构、adapter/workflow、runners、分析、导出、serial stack、产品 CLI/report/Dashboard/demo 和 public adapter 默认无需硬件；当前本地 UX 门禁完成 2,275 项回归和 11,911/11,911 三 package 语句覆盖，fresh base/serial 安装、双 demo、hosted matrix、tester journey 和 external adapter 均有可重复证据 | 较新的交互/展示/治理更改仍须在精确提交上重跑 hosted CI；保持物理 HIL 为可选路径 |
...
Software Phase 1–5 均已完成各自 8/8；Software Phase 6 已完成 7/8。Phase 5 建立统一 CLI、产品 contracts/catalog/issues、单向依赖门禁、有界 single-owner worker、显式 factories、共享 read/DC/迟滞 services、只呈现 finalized result 的确定性人类报告、可运行的六步 Dashboard workflow、12-artifact deterministic demo 和 public compatibility freeze。Phase 6 已增加 read-only hosted CI、`0.1.0b1` metadata、deterministic release verifier、初学者 tester 闭环、fresh-wheel external `python -I` public adapter proof 和 `PASS_WITH_REVIEW` final candidate audit。当前本地 UX 验证基线为 2,275 项完整回归和 11,911/11,911 正式+可选+产品 package 语句覆盖；Draft PR #7 的已提交 head 有八项 hosted CI 通过，较新的交互安全、展示和治理更改仍待精确提交/hosted 验证。两项 `VERIFIED_BENCH` 仍只属于先前的 controller UART 与 MSP profile，被验证的 AFE 硬件需求仍为 0。下一里程碑是 Software Phase 6 Step 8 owner-controlled publication/release decision；这不自动授权 Ready、合并、公开、许可、标签或 Release。
