...
Software Phase 4 Steps 1–2 add two lower communication boundaries without changing the frozen Phase 1–3 public surface. `analog_validation.transport` turns arbitrary bounded byte chunks into complete LF records and tracks profile-selected sequence widths. `analog_validation.protocol.envelope` validates printable tokens, terminators, record length, and CRC without assuming a namespace; the existing AFE `framing.py` delegates to it and adds only the historical `AFE` shape requirement. A versioned `afe-channel-map.v1` then provides explicit legacy-to-canonical channel conversion. None of these layers opens a COM port or interprets MSP430 business fields.
