...
| TD-025 | P1 | Step 3 已完成 replaceable backend/session/raw-log，Step 6 已完成 receive-only `SerialAdapter`、AFE/MSP capability projection、共用 workflow 和异常/重连链；尚无 concrete OS/pyserial backend 或 owning worker | 产品 adapter 组合已可在内存 backend 中复用，但仍不能枚举/打开真实 COM、证明 OS timing，或形成 HIL | Software Phase 4 Step 7 评审最小 OS backend，Phase 5 再处理 worker/UI；证据见 `reports/software-phase4-step3.md` 与 `reports/software-phase4-step6.md` |
