...
| TD-014 | CLOSED | 单一 `analog-validation` 已安装并提供稳定 human/JSON 的 version/profiles/ports、Simulator/Replay read/DC/迟滞、显式 receive-only observe、确定性 `report` 和六步本地 `dashboard`；仅 `demo` 作为尚未实现功能诚实返回退出码 4 | 用户可从已安装基础包运行正式软件验证、报告和 reviewed Dashboard workflow；CLI/UI 共享请求编译与 worker service，不维护第二套工程逻辑 | 证据见 Phase 5 Steps 3–6 reports、`docs/product-cli.md` 和 `docs/dashboard.md` |
...
| TD-033 | CLOSED | Phase 5 Step 1 已建立 `analog_validation_app`，删除根 `dashboard/` Python 占位与两个 legacy-only analysis tests；Steps 2–6 worker/factories/services/shared workflow/CLI/presentation/reporting/Dashboard 继续通过单向架构门禁 | 六步 UI 已通过正式 product services 接线，没有重新引入旧占位或复制工程算法 | 2,131 项完整回归、11,219/11,219 package statements 与 Phase 1–4 goldens 继续通过；见 Phase 5 Steps 1–6 reports |
...
| TD-035 | P3 | 当前正式门禁保持 11,219/11,219 语句覆盖；额外全仓分支诊断为 99.91%，有 14 条旧模块条件分支未走到，而 Step 6 新增 workflow/Dashboard 模块分支覆盖均为 100% | 没有未执行语句或已知功能失败，但“100% coverage”若不注明口径会被误解为分支覆盖也为 100% | 文档始终写明 statement coverage；在独立质量变更中评审旧分支是不可达、防御性还是需要新增测试 |
