...
| TD-025 | P1 | Step 3 已完成 replaceable backend port、发现/打开/bounded read/timeout/断线/有限重连/close 和 bounded memory-only raw log；尚无 concrete OS/pyserial backend、worker 或 SerialAdapter | host 生命周期与故障恢复可独立验证，但仍不能声称真实 COM 或产品 adapter 可用 | Software Phase 4 Step 6；Step 3 证据见 `docs/serial-transport.md` 和 `reports/software-phase4-step3.md` |
...
| TD-030 | P1 | AFE capability snapshot 仍保留冻结 wire vocabulary `adcN/dacN/pwmN/dinN`，而 telemetry/read workflow 使用 canonical `afe.chN.*`；当前 profile 尚不是 DeviceAdapter | 若 Step 6 直接暴露 wire snapshot，会造成请求通道与返回 Measurement 不一致；若静默改名，会破坏 command safety 语义 | Step 6 建立显式、测试覆盖的 adapter capability projection，同时保留 profile-native snapshot 供 command gate 使用 |
