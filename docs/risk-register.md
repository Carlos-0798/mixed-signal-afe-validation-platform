...
| Phase 5 UI duplicates profile or device business logic | Steps 1–3 product boundary implemented; factories/services/CLI and worker reject copied or reverse-imported device/analysis/serial/GUI logic | Keep CLI/worker/Dashboard above public adapters/workflows; device parsing remains in profiles and engineering math remains in the core |
| A cancelled product job leaves a thread, adapter, or COM handle alive | Generic worker cancellation/cleanup and subprocess interpreter interruption are host-tested; interactive Windows console control events, real COM disconnect, and non-cooperative third-party services are not validated | Single job owner, independent completion event, 50 ms interrupt polling, bounded I/O, cooperative token, cleanup on every post-factory path, finite join, no PASS after cleanup failure, and separate console/long-duration/physical-disconnect smoke gates |
...
| Dashboard cross-thread updates freeze or corrupt UI state | Phase 5 presenter/widgets planned | Worker publishes immutable bounded events; Tk main thread alone applies presenter state and renders widgets |
| Human report or plot silently changes an engineering result | Phase 5 report layer planned | Presentation consumes finalized result bundles, never refits/re-evaluates, and exact report/plot goldens freeze outcome and provenance |
| Local product output leaks raw frames, usernames, paths, or USB identity | Phase 5 privacy contract planned | Local-only default, sanitized summaries/hashes, no absolute paths or hardware IDs in committed artifacts, explicit raw-data opt-in, and hostile-input tests |
| Tk availability becomes a base-package installation dependency | Step 1 base-wheel CLI passed without importing Tk; no window exists yet | Keep Dashboard import lazy, retain headless base-wheel gates, add clear missing-Tk guidance and a separate real-window smoke in Steps 5–8 |
