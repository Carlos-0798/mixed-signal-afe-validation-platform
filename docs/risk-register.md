...
| Phase 5 UI duplicates profile or device business logic | Steps 1–2 product boundary implemented; worker also rejects direct device/analysis/serial/GUI imports | Keep CLI/worker/Dashboard above public adapters/workflows; device parsing remains in profiles and engineering math remains in the core |
| A cancelled product job leaves a thread, adapter, or COM handle alive | Generic worker cancellation/cleanup is host-tested; no real COM disconnect or non-cooperative third-party service is validated | Single job owner, bounded I/O, cooperative token, cleanup on every post-factory path, finite join, race/fault tests, no PASS after cleanup failure, and separate long-duration/physical-disconnect HIL |
