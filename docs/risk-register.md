...
| Phase 5 UI duplicates profile or device business logic | Step 1 product boundary implemented; duplicate protocol/analysis checks pass | Keep CLI/worker/Dashboard above public adapters/workflows; device parsing remains in profiles and engineering math remains in the core |
| A cancelled product job leaves a thread, adapter, or COM handle alive | Phase 5 worker planned, not implemented | Single job owner, bounded I/O, cooperative cancellation, `finally` cleanup, join timeout, race/fault tests, and no PASS after cleanup failure |
