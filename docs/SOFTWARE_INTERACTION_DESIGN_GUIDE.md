...
当前 Dashboard 在异常旁保留内部稳定字段键，让界面自动滚动、聚焦并高亮第一个可操作
字段，同时用文字写明字段名称。该提示不写入 `user-issue.v1`、CLI JSON、项目或 manifest；
无法可靠定位的错误继续显示问题卡片，不解析英文错误文本或猜测控件。
...
- 支持的最小窗口与常用缩放比例下，标签不截断、按钮不重叠、关键内容可滚动到达；
- 密集表单优先分行或缩短固定宽度，不用水平滚动隐藏必填项和提交动作；
- Windows 高对比度启用时服从系统窗口、文字、选中和禁用颜色，错误仍须有文字；
- Enter、Space 和原生对话框行为符合平台习惯；
- 不添加没有界面提示和文档说明的隐藏快捷键。

本项目已经在 1040×760 自动检查 Tk scaling 1.0、1.25、1.5、1.75 和 2.0、基本
焦点遍历、关键控件水平边界及标准/强制 Windows 高对比度样式。它不代表通过了
屏幕阅读器、WCAG、真实用户、Remote Desktop 或所有显示环境认证；运行中改变系统
主题后目前需要重启 Dashboard。

TD-043B2A 进一步把“键盘可达”和“辅助技术可理解”分开验收。当前 Windows 候选使用
Tk 8.6.15，没有官方 `tk accessible` 接口；UI Automation 只能把 37 个应用后代识别为
无名称 Pane。因此当前 Dashboard 明确不支持屏幕阅读器。代码只在运行时确实提供该
接口时设置控件名称、角色、帮助和动态值通知；即使适配器启用，也仍须通过 UIA、
Narrator 与真实用户测试，不能自动填写 PASS。当前文本替代路径为 CLI，但同样尚无
屏幕阅读器真实用户认证。
...
| 六列表单在高缩放下把输入压得过窄 | 新手看不完整字段和值 | 支持宽度内分为三列并增加纵向滚动，不牺牲输入可读宽度 |
| 结果表固定列宽把保存按钮推出窗口 | 最终动作不可见 | 给表格和说明文字设定视口内宽度预算，并验收所有终态按钮 |
| 固定深色覆盖 Windows 高对比度 | 系统可访问性偏好失效 | 启动时只读系统状态并使用系统色；状态和错误继续以文字表达 |
| 把 Tab 顺序通过写成“屏幕阅读器可用” | 辅助技术只能看到无名称容器，用户不知道焦点对象和状态变化 | 独立检查程序化名称/角色/值与事件；运行时缺少语义 API 时明确阻塞支持声明 |
...
6. **程序化辅助功能检查**：UI Automation 检查名称、角色、值、可聚焦控件与状态事件；
7. **端到端链路**：Simulator/Replay 从输入到 Result/Export；
8. **压力与竞态测试**：快速连续运行、有界事件队列、重复刷新和取消；
9. **安装后测试**：从 wheel 在仓库外启动，而不是只验证源码目录；
10. **人工验收**：视觉层级、语义理解、系统 picker、异常恢复、屏幕阅读器播报和主观流畅度。
