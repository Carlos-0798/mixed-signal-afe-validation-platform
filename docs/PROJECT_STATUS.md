...
**Current milestone:** Software Phase 1 — Step 7 of 8 complete<br>
...
The repository currently provides an installable, controller-neutral Python core for Analog Validation Studio. It includes explicit measurement provenance, device capabilities and safe ranges, test-run conclusion semantics, one CRC/framing implementation, the versioned AFE v1 profile, and strict non-executable JSON configuration.
...
| Package build and external install | Passed; external `validation-config.v1` round trip returned true and output remained false |
...
Software Phase 1 Step 8 will add frozen AFE business-message vectors, finish the one-time legacy migration, run the complete release-style verification matrix, and publish the Phase 1 closure report. This remains host-software work and will not create a hardware claim.
