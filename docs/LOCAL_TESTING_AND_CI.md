...
该策略同步到当前开发分支 `codex/calibration-workflow` 和默认 `main` 的 CI 配置。
main 的 CI 策略更新只更改触发入口；后续首页展示更新也不合并 Draft PR 中的产品功能。
其它历史分支保持原样；
如果将来从历史分支恢复工作，先带入并核对当前策略，不能直接推送旧的自动触发配置。
...
仓库继续私有。Ready、合并、Release、包发布、公开、许可证/历史变更及硬件操作
仍按各自授权边界执行。
