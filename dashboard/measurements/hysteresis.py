...
class HysteresisResult:
    """Legacy Phase 0 threshold result awaiting Phase 3 migration."""
...
def _transition_midpoint(samples: Sequence[Sample], before: int, after: int) -> float:
    ...
    for (previous_mv, previous_state), (current_mv, current_state) in pairwise(samples):
...
def calculate_hysteresis(
    rising_samples: Sequence[Sample], falling_samples: Sequence[Sample]
) -> HysteresisResult:
